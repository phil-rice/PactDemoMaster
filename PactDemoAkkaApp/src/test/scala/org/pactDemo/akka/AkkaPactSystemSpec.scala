...
import org.pactDemo.akka.CustomRequestObject.makeRequest
import org.pactDemo.akka.StringToObject.makeObject
...
class AkkaPactSystemSpec extends PactDemoSpec {
  ...
  /**
    * Test Case for makeObject --> StringToObject[String, T] extends (String => T)
    */
  ...
  it should "Test makeRequest : return proper JSON in request against CustomRequestObject" in{
    makeRequest(CustomRequestObject(1, "12345-valid-for-id-1-token")).contentString shouldBe("""{"id": "1", "token":"12345-valid-for-id-1-token"}""")
  }

  it should "Test makeRequest : return token value from JSON in request against CustomRequestObject" in{
    makeRequest(CustomRequestObject(1, "12345-valid-for-id-1-token")).contentString.token shouldBe("12345-valid-for-id-1-token")
  }

  it should "Test makeRequest : return id value from JSON in request against CustomRequestObject" in{
    makeRequest(CustomRequestObject(1, "12345-valid-for-id-1-token")).contentString.id shouldBe(1)
  }

  it should "Test makeRequest : return 0 as id value from JSON in request against CustomRequestObject" in{
    makeRequest(CustomRequestObject(0, "12345-valid-for-id-1-token")).contentString.id shouldBe(0)
  }

  it should "Test makeRequest : return empty as token value from JSON in request against CustomRequestObject" in{
    makeRequest(CustomRequestObject(0, "")).contentString.token shouldBe("")
  }

  it should "Test makeRequest : return ContentType of a request as hcl.token" in{
    makeRequest(CustomRequestObject(0, "")).headerMap.get("ContentType").get shouldBe("application/hcl.token")
  }

  it should "Test makeRequest : return method name of a request as post method" in{
    makeRequest(CustomRequestObject(0, "")).method.name shouldBe("POST")
  }

  it should "Test makeRequest : return uri of a request as set in post method" in{
    makeRequest(CustomRequestObject(0, "")).uri shouldBe("/token/android/post")
  }

  it should "Test makeRequest : throws JsonMappingException for empty CustomRequestObject" in{
    assertThrows[JsonMappingException]( makeRequest( "" ) )
  }

  it should "Test makeRequest : throws NullPointerException for null input" in{
    assertThrows[NullPointerException]( makeRequest( null ) )
  }
